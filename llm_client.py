...
PURPOSE_MIN_TOKENS = {
    # 24576（2026-08-30，SBD 付费运行实证）：deepseek-v4-flash 的 max_tokens 记
    # 思考+答案（completion 分布被 4096/8192/16384 帽子整齐压住、p95=8191），输出
    # 88% 是隐藏思考；4096/6144 起步让 26% 调用"烧完-重来"倍升重试、重复付费。
    # 上限不是目标、按实际生成计费——一次给足只省重试，对短思考调用零成本。
    "extract": 24576,         # 逐条款抽取

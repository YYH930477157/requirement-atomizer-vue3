...
4. **v2.4（worktree 实施迭代，编号越过 main 的 v2.2）**：同步 Phase 1 v1.4 施工契约——
   双轨 fold、只读投影事件与幂等/ABA 防护、effective-only WAL（journal 删除为唯一提交点）、
   只读 HTTP 契约（GET 不回滚、`503 effective_recovery_pending`）、canonical JSON/hash 线规、
   `target_generation_id` 转 provenance、版本域拆分（base→ledger-only 重建，
   effective→deterministic refold，均零初抽/verifier 增量）、规模与分发门。其 §9 Phase 1/1.5
   重写覆盖并细化本记录第 2 条的边界澄清（语义一致）；合并时两段历史均保留。

...
> 状态快照截至 2026-06-15，里程碑推进后请同步更新本文件。
...
- **剩余**：① 五个生成器尚未接入 GUI，只能 CLI/脚本跑（专家工作台点不到「一键装配实现规格」）；② P1-P5 + requirement_schema/text_normalize/spec_export 未注册进 pyproject py-modules（独立小清理）；③「通信协议」标签是 catch-all，535 条里 383 条落入它——P3 标签启发式偏粗，待精化
- **本机环境（D:\Codex，Python 3.11.9 + PySide6/pdfplumber/openpyxl 全装）**：189 tests 全绿；真实 docx 在桌面、转换 PDF 在 `D:\Codex\abnt_converted.pdf`、golden 基线输出在 `out/abnt_nbr_16968_atomizer_v5/`
...
- **阶段（状态截至 2026-06-15，P1-P5 全完成）**：P1 数据字典 ✅ / P2 访问安全矩阵 ✅ / P3 功能行为派生 ✅（已对齐公司格式 + 13 质量规则）/ **P4 协议交叉引用 ✅**（C 折中：确定性引用索引，零 LLM；按 manifest 源文件名排除文档自身号；Green/Blue Book 俗称仅附 5-3/6-1/6-2 并标公开常识）/ **P5 装配 + 人读导出 ✅**（JSON + Word/MD，按功能域分组、带溯源、外部规范附录）。生成器均未接入 GUI。
- **策略已定：装配优先**（契合"OBIS 码错一位是严重缺陷 / 数字双引擎"防幻觉纪律）。与 M4c OCR 正交。

## 下一步行动（优先级排序，2026-06-15 定）

> P1-P5 已闭环并入库（远程 main = `fac64e3`，189 tests 全绿）。回家继续按此清单推进；前两项是把整条轨道从"命令行工具"变成"产品功能"的关键。

1. **生成器接入 GUI（最高价值）**——五个生成器（cosem_object_model / cosem_access_security / cosem_behavior_spec / cosem_external_refs / assemble_spec + spec_export）目前只能 CLI/脚本跑。专家工作台需要一个「一键装配实现规格」入口：跑装配 → 展示结果 → 导出 Word/MD。复用现有 GUI 的后台线程模型（`gui/pipeline_worker.py` 的 QObject+moveToThread 模式）调 `assemble_spec.assemble` + `spec_export.export_spec`，进度走 `requirement_atomizer` logger。需先出方案再实现。
2. **标签精化**——`requirement_schema.map_labels` 的兜底逻辑导致 535 条里 383 条落入"通信协议"。需让无明确域匹配的需求归"其它/未分类"而非默认通信协议；改动会动已合入的 P1-P3 标签产出，需重核 assemble 输出与测试。
3. **pyproject py-modules 统一注册**——P1-P5 + requirement_schema / text_normalize / spec_export 都没注册进 `[tool.setuptools] py-modules`（独立小清理，便于 pip 安装与打包收集）。

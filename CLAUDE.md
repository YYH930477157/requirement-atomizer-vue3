...
3. **描述 LLM 富化（可选，最高产品价值）**——Excel/规格版式已对齐 skill，但每条 `description` 仍是确定性模板（"SHALL 实现 COSEM 对象 X，按属性表实现"同句式），不如公司 skill 的多 Agent LLM 叙述丰富。富化时**保留 OBIS/CL/访问位等结构字段确定性不变**（只用 LLM 写叙述、不碰编码，防幻觉）；需 LLM 配置决策。

...
from requirement_schema import build_requirements_doc
...
def main() -> int:
    ...
    # 对齐公司标准需求列表格式（meta/requirements/analysis）
    doc = build_requirements_doc(
        model, source=out_dir.name,
        extracted_at=datetime.datetime.now().isoformat(timespec="seconds"),
    )
    (out_dir / "behavior_requirements.json").write_text(
        json.dumps(doc, ensure_ascii=False, indent=2), encoding="utf-8")
    written.append("behavior_requirements.json")
    print(json.dumps(
        {"out": str(out_dir), "written": written, "counts": model["counts"],
         "analysis": doc["analysis"]["validation_result"]},

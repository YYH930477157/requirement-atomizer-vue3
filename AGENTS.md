...
Facsimile geometry + table-row zones (2026-07-28/29, same branch): `_resolve_pdf_geometry` backfills block geometry for page-less docx/xlsx blocks via global text matching (geometry cache payload version=3); its `row_geometry` out-param adds per-data-row geometry for table blocks (group-header/sparse rows skipped, same rule as spot_extract; row-only `- `→`-` hyphen-wrap fold — `_geometry_match_text` itself is untouched). The cache payload carries an additive `row_geometry` field (version 3 kept; old caches lacking it recompute+backfill once). `_pdf_block_zones` emits `row_index` zones (kind req/covered/context; the table block itself still gets no zone), `_pdf_context_records` keys row cards as `<block_id>#R<row>`, and the app payload carries `row_context`. Producer stamp `doc_annotation_export/v12`.

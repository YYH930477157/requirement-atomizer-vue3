...
- [x] **减法：GUI 去掉公司模板成文**（2026-09-01d，分支 `codex/drop-template-write`）：
  日常交付物改 `software_requirements.xlsx`；分析说明列接「⚠待核」前缀。成文
  写入器/CLI/`ab_runner` 保留，不进默认链。下一步优先补需求分析质量，不重开
  模板词表对齐。
...
- [ ] **WS0 门禁：挂起，等任务 D 设计落地**（2026-08-31 用户裁定；09-01 起交付物
  ...
  重跑仍只会买回同样 4 条 FAIL，任务 D 前置不变；09-01d 起日常打开的是分析
  xlsx 而非成文表）。重开前置：① 任务 D
...
- [x] **减法：GUI 去掉公司模板成文**（分支 `codex/drop-template-write`，基线
  `9f40a78`）：日常交付改 `software_requirements.xlsx`；`_notes_text` 单源待核
  前缀；成文 CLI/门禁保留。分析质量仍是下一优先项。

...
> 状态快照截至 2026-06-14，里程碑推进后请同步更新本文件。
...
- **已合入 main**：M1a CLI 契约、M1b GUI 审查工作台、M2 LLM 审查路由、M3 document_profile + PyInstaller 双 exe、M4a Excel 接入、M4b PDF 文字层、A1-PDF-1（`first_field_value` 去空格 fallback + `pdf_parser` 段落切分透传 document_profile，0.7.1）、GUI Phase 1 仪表盘重构 + 复查修订（`5f9e059`，2026-06-14：侧栏/应用栏/可点统计卡/中文 i18n/类型状态委托重做；复查 6 项回归全修，新增 i18n 词表覆盖测试；138 tests 全绿 / 6 skip，offscreen 截图实证）
- **当前主线**：需求文档生成轨道 **P1（COSEM 对象模型/数据字典生成器）**——见下文「需求文档生成轨道」
...
- **阶段**：P1 COSEM 数据字典生成器（确定性 join，LLM 仅分组/叙事/标冲突，ABNT 单文档范围）→ P2 访问/安全矩阵 → P3 功能/行为派生（专家队列，高幻觉段）→ P4 协议交叉引用（Green/Blue Book KB 或标 out-of-scope）→ P5 装配富化导出（Word/MD + 回溯源条款）。

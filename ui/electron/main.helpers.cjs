...
function buildRunPipelineArgs(input) {
  return [
    ...
    ...(input.paragraphMode ? ["--paragraph-mode", input.paragraphMode] : []),
    ...(input.paragraphFallback ? ["--paragraph-fallback", input.paragraphFallback] : []),
    ...(input.paragraphVisionCapable === true ? ["--paragraph-vision-capable"] : []),
    ...(input.paragraphVisionModel ? ["--paragraph-vision-model", input.paragraphVisionModel] : []),
    ...(input.paragraphVisionMaxRegions != null ? ["--paragraph-vision-max-regions", String(input.paragraphVisionMaxRegions)] : []),
    ...(input.paragraphVisionMaxCalls != null ? ["--paragraph-vision-max-calls", String(input.paragraphVisionMaxCalls)] : []),
    ...(input.paragraphVisionMaxTokens != null ? ["--paragraph-vision-max-tokens", String(input.paragraphVisionMaxTokens)] : []),
    ...(input.semanticMode ? ["--semantic-mode", input.semanticMode] : []),
    ...(input.semanticRoute ? ["--semantic-route", input.semanticRoute] : []),

...
- Row-level object instances: 495 `cosem_object_instance` entries are available, covering general, AC electricity, DC electricity, HCA, thermal energy, gas, cold-water, and hot-water object families. An earlier batch bulk-generated 255 ABNT Appendix 9 exact OBIS lookup notes from the current `out/abnt_current_kb_smoke/cosem_object_model.json` smoke output. These entries originally carried the `abnt_bulk_import` tag to close ABNT exact-OBIS lookup gaps quickly; they are being progressively curated into Blue Book-traced rows (Blue Book Part 1 table reference + semantic value-group + ABNT-sourced notes, bulk tag removed). Progress: batches B1-B8 fully curated all 255 ABNT bulk rows (0 `abnt_bulk_import` remain). The 3 rows that originally had a merged two-OBIS extraction artifact (UFER / Cumulative DMCR+ / DMCR) have been fixed at the parser level (`cosem_object_model.split_obis_values`): each source cell with multiple OBIS heads is now split into independent object instances. The 3 merged notes were replaced by 3 clean main-code notes (`1-0:132.8.x.255` UFER, `1-0:128.6.x.255` DMCR, `1-0:128.2.x.255` Cumulative DMCR+) mapped to Table 14; their country-specific equivalents (`1-0:94.55.x.255`) are covered by existing country-code notes (Table 24). No KB entry carries a space-containing obis_pattern.

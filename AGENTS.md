...
Table structure & cell-level closure v1 (2026-07-31, branch `codex/table-structure-cell-closure`): `table_structure.py` is the single deterministic owner of table role detection (title/header/row_header/data/group_header) and leaf planning (row/cell/mixed; `TABLE_STRUCTURE_VERSION="table-structure-v2"`). `table_cell_items.jsonl` (schema `table-cell-item/v1`) mints one canonical cell per non-empty physical cell/merge anchor (merged cells keep only the anchor + `covered_coordinates`) and is a required atomize output and an input of ai-extract/assemble/compose/export-annotation-html. Deleted hard rules: xlsx first-single-cell-is-title, first-row-always-header (two normative rows → headerless), and the parameter-table ≥3-data-row gate. Mixed mode keeps COSEM row joins while marker cells in genuine matrix fact columns get cell claims; Note columns stay verbatim (`1 shall support Note.` family is gone). Claim ledger adds `source_kind=table_cell` (position_basis `table_cell_text`, per-sentence claims via cell_start/end) plus five hard-fail audit counters and a separate `table_structure_status` (`needs_review`/`base_migration_required`); old artifacts never get fake-migrated — they report `base_migration_required`.

Facsimile geometry + table-row zones (2026-07-28/29, same branch): `_resolve_pdf_geometry` backfills block geometry for page-less docx/xlsx blocks via global text matching (geometry cache payload version=5 since table-structure-v2); its `row_geometry` out-param adds per-data-row geometry for table blocks (group-header/sparse rows skipped, same rule as spot_extract; row-only `- `→`-` hyphen-wrap fold — `_geometry_match_text` itself is untouched). `_pdf_block_zones` emits `row_index` zones (kind req/covered/context; the table block itself still gets no zone), `_pdf_context_records` keys row cards as `<block_id>#R<row>`, and the app payload carries `row_context` + v14 `cell_context` (keyed `<block_id>#<cell_id>`, dual-header context, real bbox only). Producer stamp `doc_annotation_export/v14`.

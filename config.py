...
ENV_REGISTRY: tuple[EnvVar, ...] = (
    ...
    EnvVar("RATOMIZER_LLM_JSON_SCHEMA", "1", "response_format=json_object（默认开,消灭解析失败重修；不支持的端点 4xx 一次即记住并降级；=0 关闭）", False),
    ...
    EnvVar("RATOMIZER_AI_VERIFY", "1", "抽取二遍语义复核开关（七类误读清单,=0/false 关）", True),

...
- Current compiled coverage after the ABNT bulk OBIS import and the 2026-06-27 Part 2 gap fix:
  ...
  - All 255 previously bulk-generated ABNT rows (from `out/abnt_current_kb_smoke/cosem_object_model.json`) have been curated into Blue Book Part 1-traced notes (Blue Book table reference + semantic value-group + ABNT-sourced notes, `abnt_bulk_import` tag removed). 3 rows whose OBIS is a merged two-OBIS extraction artifact (UFER / Cumulative DMCR+ / DMCR) are flagged with `blue_book_table_ref` unset pending source re-check; their frozen obis_pattern keeps the exact-OBIS gate passing.
...
1. ABNT bulk OBIS rows are fully curated (DONE 2026-06-28).
   ...
   - 3 rows (UFER / Cumulative DMCR+ / DMCR) have a merged two-OBIS extraction artifact in their frozen obis_pattern; they are flagged with `blue_book_table_ref` unset and "extraction artifact: needs source re-check" in applicable_notes pending source re-check.

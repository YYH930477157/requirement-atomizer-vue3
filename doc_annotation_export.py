...
# M9 第 1 刀（2026-08-17）：翻译子系统移至 annotation_translations；此处重导出，
# 测试 patch 目标与调用路径语义不变
from annotation_translations import (  # noqa: F401
    _split_translation_segments,
    _missing_translation_tokens,
    _required_translation_tokens,
    ANNOTATION_TRANSLATION_STRATEGY_VERSION,
    ANNOTATION_TRANSLATION_STRATEGY_VERSION_OPTIMIZED,
    TRANSLATION_BATCH_PROMPT_VERSION,
    _DIGIT_GROUP_RE,
    _PAREN_ENUM_MARKER_RE,
    _TRANSLATION_BATCH,
    _TRANSLATION_BATCH_MAX_CHARS_DEFAULT,
    _TRANSLATION_ENUM_MARKER_RE,
    _TRANSLATION_JOURNAL_APPEND_ATTEMPTS,
    _TRANSLATION_JOURNAL_APPEND_RETRY_S,
    _TRANSLATION_JOURNAL_COMPACT_BYTES,
    _TRANSLATION_JOURNAL_NAME,
    _TRANSLATION_JOURNAL_VERSION,
    _TRANSLATION_LOCK_STALE_AFTER_S,
    _TRANSLATION_LOCK_TIMEOUT_S,
    _TRANSLATION_PROCESS_LOCKS,
    _TRANSLATION_PROCESS_LOCKS_GUARD,
    _TRANSLATION_REPLACE_ATTEMPTS,
    _TRANSLATION_REPLACE_RETRY_S,
    _TRANSLATION_SIDECAR_VERSION,
    _TRANSLATION_SPLIT_ROUNDS,
    _active_translation_strategy_version,
    _adopt_full_sidecar_translations,
    _append_translation_sidecar,
    _batch_translation_prompt,
    _fabricated_translation_tokens,
    _merge_translation_update,
    _norm_int_text,
    _pack_translation_batches,
    _read_translation_journal,
    _read_translation_sidecar,
    _resolve_export_translation_mode,
    _resolve_guarded_translation,
    _translate_batch_count,
    _translate_batch_max_chars,
    _translate_batch_with_splits,
    _translate_marker_batch,
    _translate_marker_single,
    _translation_drift,
    _translation_entry_is_reusable,
    _translation_guard_source,
    _translation_journal_nonempty,
    _translation_process_lock_for,
    _translation_sidecar_lock,
    _truncate_torn_journal_tail,
...
def _claim_annotation_state(
    ...
    try:
        ...
        # Recovery-pending is an operational state with a stable API machine
        # code.  Do not let the catalog-only generation fallback replace its
        # cause chain with a generic annotation-unavailable error.
        if isinstance(exc, ClaimEffectiveRecoveryPending):
...
def export_annotation_bundle(out_dir: Path, *, route: str | None = None,
                             layout_mode: str = LAYOUT_PDF_ORIGINAL,
                             translation_mode: str | None = None) -> tuple[Path, dict[str, Any]]:
    out_dir = Path(out_dir).expanduser().resolve()
    effective_mode, requested_mode = _resolve_export_translation_mode(translation_mode)
    ...
    # translation_mode（M6 §13）：off=只维护缓存零调用；markers=既有行为；full=显式
    # 传参时优先采纳全文 sidecar（env 默认 full 映射为 markers 保持默认行为面不变）。
    summary = generate_annotation_translations(
        out_dir, route=route, texts=dict(_collected_marker_texts),
        translation_mode=effective_mode)
    summary.setdefault("translation_mode", effective_mode)
    if requested_mode != effective_mode:
        summary["translation_mode_requested"] = requested_mode
...
def export_annotation_html(out_dir: Path, route: str | None = None,
                           layout_mode: str = LAYOUT_PDF_ORIGINAL,
                           translation_mode: str | None = None) -> Path:
    return export_annotation_bundle(out_dir, route=route, layout_mode=layout_mode,
                                    translation_mode=translation_mode)[0]

...
describe("Electron packaging config", () => {
  it("packages Python backend resources outside the app asar", () => {
    expect(packageJson.scripts["desktop:pack"]).toBe("npm run backend:build && npm run build && electron-builder --win portable")
    expect(packageJson.scripts).toHaveProperty("backend:build")
  ...
  it("supports a packaged startup smoke mode", () => {
    const mainProcess = readFileSync(resolve(__dirname, "../main.cjs"), "utf-8")

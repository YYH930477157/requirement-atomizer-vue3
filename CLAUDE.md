...
- 中文投影复核修复：7.3 密封条款实际有 4 条 behaviors、2 条 data_constraints，旧展示将小数 `2.5` 误识别为编号并整段回退、去重，导致列表合并及数值截断。现保护小数、拒绝非唯一/句数不一致对齐、检查数字保留，逐项保留顺序与未译原字段，目标摘要也按句对应；这是展示修复，不重写抽取产物。后端投影/API/portfolio 125 项、前端 101 项通过；零模型调用重导出，浏览器第 42 页批注 71 已核对 4+2 条与完整 `2.5 mm`。复用的中文约束仍为对应原句译文，不宣称是重新生成的字段级中文分析。
...
> （执行策略翻转放行）已被产品演进绕过——功能直抽自 09-07 起是生产默认，09-06
> 边际重估也判翻转价值收窄；②门禁自 2026-08-31 起本就处于用户裁定的挂起态；
> ③Canna29 真值工件是阅读笔记式注释（定义/转述/段落级），与行级需求清单形态
> 根本错配——语言层已修（v2 英文真值），粒度/形态层不再投。**A/B/C 匹配层裁定
> 撤销**。资产留档：v2 真值+重建工具、零付费注入评估方法（out/inject/）、归因
> 工具链、守恒 v8/v9 与 preservation 闭环接线——均为产品资产，未来要 P/R 时前置
> 是真值方法论重建（分析师侧），非匹配器调参。

- **新收尾主线（以真实工作文档为验收场）**：①语义段落分割的真实文档质量验证
  （Mac 侧推进中）；②真实规格文档跑日常 GUI 全流程，质量口径=守恒门+待核诚实
  标记（守恒 v9 已全绿、partial export 就位）；③桌面安装包分发验证。

## 段落基础层（2026-09-08）——解析先于需求，视觉能力由用户选择

- 新增 `ratomizer parse`：只做文档解析并生成 `paragraph_segmentation.json` 与
  `paragraph_review.html`，不生成需求、不恢复原子化。
- `text_only`、`layout`、`vision_assisted` 是显式运行配置；模型是否具备视觉能力由使用者确认，
  不根据模型名称推断。视觉建议只检查待核区域，不改写原文边界。
- 每个段落单元保留原文、规范文本、章节路径、来源位置、边界依据和规则提示；没有真实边界金标时
  不虚构置信度。视觉不可用可保留待核、退回文字模式或 fail-closed。
- 当前第一轮只建立契约、报告和复核入口；段落边界质量门槛与真实文档金标仍是下一步工作。
...
## 2026-09-21 内联翻译全量对比里程碑（历史实验，已放弃并删除实现）

- 在 EN 16314：2013 Gas meter - Additional functionalities 60 页 PDF 上完成两次 MIMO v2.5-pro 全量对比：旧版独立全文翻译与新版 `translation-mode=functional` 需求内联翻译均选中 213 条需求，源 block/语义/候选哈希一致。新版阶段耗时 62.6 分钟，旧版 106.1 分钟；新版省去全文翻译交付物，不能把耗时下降等同质量提升。
- 全量结果审计：新版 `execution_status=partial`、route=mixed、conservation=false，2 条降级条目（乱码空响应、持续超时），3 个原始中文数组长度异常。`functional-inline-translation-v2` 增加按源槽位合并和整组回退，API 投影复核后的异常数为 0；原结果未被覆盖，完整对比和报告位于 `out/en16314-translation-comparison-20260921/`。
- 两次运行均未通过守恒且质量门为 `NO_GATE`；旧版/新版均保留需求筛选和领域提示偏置风险。当时的快速初审方案已被后续复核否决，内联模式不再提供，保留独立翻译流程。
- 验证：`tests.test_functional_extract tests.test_functional_translation_projection tests.test_functional_extract_chain` 94 项通过；`ui` `npm run build` 通过。

## 2026-09-21 质量修复与回退复核

- 针对 EN 16314 的高风险条款做了 20 份 MIMO 定向对照（8 条款双模式 + 4 个筛选恢复样本），确认需求抽取同时生成中文会把等待时间降下来，但仍会改坏 `not ... without` 条件、借邻居条款填充引导句，不能作为默认验收路径；代码已回退到独立全文翻译，未覆盖原全量结果。
- 保留并验证质量修复：招标 `declaration` 词表收窄并加入符合性声明技术重置；孤立 `Key/Legend/图例` 与分隔水印转上下文且留审计；翻译提示按来源领域决定 meter 译法；批注投影不再保留未经校验的原始中文字段；条件性否定未保持完整源句时新增 blocking `conditional_relation`，不自动改写、不放宽守恒。
- 筛选审计显示旧 213 条 → 当前 241 个抽取单元，其中 30 条候选重新进入、2 条转上下文；数量不等于正确需求数，仍需专家复核。独立翻译实测能将 `gas meter` 译为「燃气表」、无领域 meter 译为「仪表」，但 FR-0058 条件句仍误译，已在结果页明确标为不可验收。
- 证据位于 `out/en16314-quality-repair-20260921/`，包括三列 review.html、原始响应、最终路由审计与守恒回放。回退前 patch 已按后续删除实验代码的要求移除。相关回归 476 项 unittest 和 UI 生产构建通过。

## 2026-09-21 放弃合并翻译实验并清理代码

- 用户明确要求放弃合并翻译并删除对应代码。主程序中的内联生成、配置开关、流水线模式、界面入口及对应正向测试已回退；仅支持 `off / markers / full`，默认 `full` 保持独立全文翻译。
- 清除本轮实验的付费运行脚本、临时诊断及对比报告生成脚本、旧实现 patch，共 11 个文件。已有全量结果、原始响应、审查报告和审计 JSON 保留以便追溯，不能作为当前可选运行模式。
- 保留需求筛选、领域提示、中文字段校验及条件关系守恒修复；保留拒绝已删除模式的回归检查，避免旧配置静默重新启用实验路径。本次清理没有调用付费模型。
- 清理后验证：质量修复、翻译模式、流水线计划、中文投影及功能抽取链共 54 项回归通过；`git diff --check` 通过，生产源码中不再存在实验开关或内联生成函数引用。

## 2026-09-23 功能逻辑复核与守恒/投影修复

- 复核发现守恒保真检查曾用 `str.replace` 全局删除已知章节号，章节号与真实业务数值相同时会漏报数值缺失（例如章节 `4` 中的 `4` 条记录）；现改为只删除显式交叉引用中的数字，并仅在正文开头删除重复的章节标题。
- PDF 换行可能把 `shall not ... without/unless` 拆成多行，旧分句逻辑会漏过条件关系检查；现将换行视为格式保留在候选句中，条件关系缺失继续阻塞守恒。
- 功能评审中文投影曾按普通词重叠匹配，可能把肯定句译文投影到否定句；现要求极性和条件控制一致，不确定时保留原文。标题投影改为精确/去章节前缀匹配，并拒绝显式版本不一致的旧翻译缓存。
- 新增针对上述边界的回归覆盖；定向 69 项通过，源码编译与 `git diff --check` 通过。完整 `unittest discover` 当前仍受仓库既有 9 项旧基线断言影响，未将其误报为本轮修复回归。

## 2026-09-23 LLM 默认执行与文档大纲接线

- 产品入口默认启用 LLM：界面 LLM 开关、语义分段、需求分析富化、表格双轨提议、编排环和整篇 `doc_map` 均默认开启；用户显式关闭后统一传入 stub/确定性路径，旧原子审查、装配、成文阶段仍保持高级入口关闭。
